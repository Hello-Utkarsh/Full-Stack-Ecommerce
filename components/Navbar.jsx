...
const Navbar = () => {
  ...
  return (
    <nav className='bg-[#241834] flex sticky top-0 z-10 justify-around items-center h-24 w-full'>
      <div className='flex justify-between items-center w-full mx-8 max-[500px]:mx-2'>
        <Link href={"/shop"} className='cursor-pointer'>
          <h1 className='text-4xl font-bold text-[#E4E2EA] max-md:text-3xl max-[550px]:text-2xl'>TechHive</h1>
        </Link>
        <div className='w-2/6 flex justify-center bg-[#E4E2EA] items-center rounded-md
        max-sm:h-7 max-[450px]:w-24'>
        ...
        {cookie ? <div className='flex justify-end items-center w-44 mr-4 max-sm:w-32 max-sm:mr-0'>
        ...
        </div> : <div className='flex justify-between items-center w-44 max-md:w-40 max-sm:w-32'>
          <Link href={"/shop/login"} className='mx-auto'>
            <motion.button
              whileHover={{ scale: 1.1 }}
              transition={{ type: "spring", stiffness: 400, damping: 20 }}
              className="cursor-pointer text-lg text-[#241834] bg-[#E4E2EA] font-medium rounded-xl w-fit px-3 py-2 mx-auto max-md:text-base 
              max-sm:text-sm max-sm:px-2 max-sm:py-1">Signin</motion.button>
          </Link>
          <Link href={"/shop/signin"} className='mx-auto'>
            <motion.button
              ...
              transition={{ type: "spring", stiffness: 400, damping: 20 }}
              className="cursor-pointer text-lg text-[#241834] bg-[#E4E2EA] font-medium rounded-xl w-fit px-3 py-2 mx-auto max-md:text-base
              max-sm:text-sm max-sm:px-2 max-sm:py-1">SignUp</motion.button>
          </Link>
        </div>}
